...
3. **Comprehension module** (the other 50 pts), per `docs/COMPREHENSION-DESIGN.md`. **word-meaning (picture-match) + sentence-comp + text-comp (30/50) built 2026-07-31** — 102 real RT items (9 exam years) + 12-image picture-word match, now feeding the same XP/leaderboard (not a silo, see below). **⚠️ Apply `scripts/sql/comprehension_events.sql` on the Mac mini before this goes live** — word_id needs to go nullable; local dev confirmed it's currently NOT NULL (`git log` 2026-07-31 for the probe). **Remaining: picture-retell (10/50)** — needs image assets, same cost class as the word-picture set (~$0.13, gpt-image-1-mini) and no image-gen script is committed to the repo yet. **Also open: `ครูกาญ review pass`** on the 102 extracted items before real student testing (flagged in `lib/comprehensionItems.ts` header) — OCR+agent-cleaned, answers cross-checked vs each year's official key, but not human-reviewed yet.
